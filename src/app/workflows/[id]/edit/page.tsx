...
import { ArrowLeft, Upload, Image as ImageIcon, Video, FileText, ChevronRight, ChevronLeft, Check, Loader2, Save } from "lucide-react";
...
export default function EditWorkflowPage() {
    ...
    const fetchWorkflow = async () => {
        try {
            ...
            if (!user || workflowData.userId !== user.id) {
            ...
            setCategoryId(workflowData.categoryId || '');
            setIsPaid(workflowData.isPaid);
            setIsPrivate(workflowData.isPrivate);
            setPrice(workflowData.price?.toString() || '');
            setHowItWorks(workflowData.howItWorks || '');
            setStepByStep(workflowData.stepByStep || '');
            setPosterImage(workflowData.posterImage || '');
            setYoutubeUrl(workflowData.youtubeUrl || '');

            // Handle JSON content
            if (workflowData.jsonContent) {
                setJsonInput(workflowData.jsonContent);
                setInputMethod('paste');
                validateJson(workflowData.jsonContent);
            } else if (workflowData.jsonUrl) {
                setJsonUrl(workflowData.jsonUrl);
                setInputMethod('url');
            }
            ...
            if (workflowData.tags) {
                try {
                    const parsedTags = JSON.parse(workflowData.tags);
            ...
            // Handle media
            if (workflowData.screenshots) {
                try {
                    const parsedScreenshots = JSON.parse(workflowData.screenshots);
                    setScreenshots(Array.isArray(parsedScreenshots) ? parsedScreenshots : []);
                } catch {
                    setScreenshots([]);
                }
            }

            if (workflowData.demoImages) {
                try {
                    const parsedDemoImages = JSON.parse(workflowData.demoImages);
                    setDemoImages(Array.isArray(parsedDemoImages) ? parsedDemoImages : []);
                } catch {
                    setDemoImages([]);
                }
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (inputMethod === 'paste') {
                formData.append('jsonContent', jsonInput);
                formData.append('jsonUrl', ''); // Clear URL if using paste
            } else {
                formData.append('jsonUrl', jsonUrl);
                formData.append('jsonContent', ''); // Clear content if using URL
            }
            ...
            <MainLayout>
                <div className="container mx-auto px-4 py-8 max-w-6xl">
                    <div className="text-center">
                        ...
                        <Button asChild>
                            <Link href="/workflows/my">Back to My Workflows</Link>
                        </Button>
                    </div>
                </div>
            </MainLayout>
    ...
    return (
        <MainLayout>
            <div className="container mx-auto px-4 py-8 max-w-6xl">
                <div className="mb-8">
                    <Link href={`/workflows/${workflow.id}`} className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground mb-4">
                        <ArrowLeft className="mr-2 h-4 w-4" />
                        Back to Workflow
                    </Link>
                    <h1 className="text-3xl font-bold">Edit Workflow</h1>
                    <p className="text-muted-foreground mt-2">
                        Update your workflow details and content
                    </p>
                </div>
                        ...
                        <div className="space-y-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Update essential details about your workflow
                                    </CardDescription>
                                </CardHeader>
                        ...
                        <div className="space-y-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Update visual content to showcase your workflow
                                    </CardDescription>
                                </CardHeader>
                        ...
                        <div className="space-y-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Update your N8N workflow JSON content
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-6">
                        ...
                        <div className="space-y-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Update documentation to help users understand your workflow
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-6">
                                    <div className="space-y-2">
